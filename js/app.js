...
function formRating() {
  ...
  function initRating(rating) {
    const ratingValue = parseFloat(rating.querySelector('.rating__value').textContent);
    const ratingItems = rating.querySelectorAll('.rating__item svg');

    // Устанавливаем заполнение звезд на основе значения
    setRatingStars(ratingItems, ratingValue);

    // Если присутствует класс 'rating_set', добавляем возможность установки рейтинга
    if (rating.classList.contains('rating_set')) {
      enableRating(rating, ratingItems);
    }
  }

  function setRatingStars(ratingItems, value) {
    ratingItems.forEach((star, index) => {
      const fullStars = Math.floor(value); // Целое число для полностью заполненных звёзд
      const fractionalPart = value - fullStars; // Дробная часть для текущей звезды

      if (index < fullStars) {
        star.style.fill = "#ffb006"; // Полностью заполненная звезда
      } else if (index === fullStars && fractionalPart > 0) {
        const gradientId = `gradient-${index}`;
        createDynamicGradient(star, fractionalPart, gradientId);
        star.style.fill = `url(#${gradientId})`;
      } else {
        star.style.fill = "#939393"; // Пустая звезда
      }
    });
  }

  function createDynamicGradient(star, fraction, gradientId) {
    const svgNamespace = "http://www.w3.org/2000/svg";

    // Проверяем, существует ли уже градиент, если нет — создаём его
    ...
    if (!gradient) {
      ...
      stop1.setAttribute("stop-color", "#ffb006"); // Заполненный цвет

      const stop2 = document.createElementNS(svgNamespace, "stop");
      stop2.setAttribute("offset", `${fraction * 100}%`);
      stop2.setAttribute("stop-color", "#939393"); // Пустой цвет

      gradient.appendChild(stop1);
      gradient.appendChild(stop2);
      document.querySelector("svg defs").appendChild(gradient);
    }
  }

  function enableRating(rating, ratingItems) {
    ratingItems.forEach((star, index) => {
      star.addEventListener("mouseenter", () => setRatingStars(ratingItems, index + 1));
...
;// CONCATENATED MODULE: ./src/js/files/range.js



const rangeSlider = document.querySelectorAll("[data-range]");
if (rangeSlider.length) {
  rangeSlider.forEach(rangeItem => {
    const inputRangeFrom = rangeItem.querySelector("[data-range-from]");
    const inputRangeTo = rangeItem.querySelector("[data-range-to]");
    const inputRangeItem = rangeItem.querySelector("[data-range-item]");
    const rangeInputs = [inputRangeFrom, inputRangeTo];
    nouislider.create(inputRangeItem, {
      start: [Number(inputRangeFrom.value)],
      connect: "lower",
      range: {
        'min': Number(inputRangeFrom.dataset.rangeFrom),
        'max': 100000,
      }
    });
    inputRangeFrom.addEventListener("change", setPriceValues);
    function setPriceValues() {
      let priceStartValue;
      if (inputRangeFrom.value != "") {
        priceStartValue = inputRangeFrom.value;
      }
      inputRangeItem.noUiSlider.set([priceStartValue]);
    }
    inputRangeItem.noUiSlider.on("update", function (values, handle) {
...
function initSliders() {
  ...
  if (document.querySelector('.slider-hero')) {
    ...
    new Swiper('.slider-hero__slider', { //Указываем класс нужного слайдера
      ...
      */
      ...
      // Кнопки "влево/вправо"
      navigation: {
        prevEl: '.slider-hero .arrows-slider__arrow_prev',
        nextEl: '.slider-hero .arrows-slider__arrow_next',
      },
  ...
  if (document.querySelector('.products-block')) {
    //Создаем слайдер
    new Swiper('.slider-products__slider', { //Указываем класс нужного слайдера
      //Подключаем модули слайдера
      //для конкретного случая
      ...
      scrollbar: {
        el: '.slider-products__scrollbar',
        draggable: true,
      },
      // Кнопки "влево/вправо"
      navigation: {
        prevEl: '.products-block .arrows-product-block__arrow_prev',
        nextEl: '.products-block .arrows-product-block__arrow_next',
      },
  ...
  if (document.querySelector('.slider-preview-products')) {
    new Swiper('.slider-preview-products__slider', {
      modules: [Navigation, A11y, Scrollbar],
      observer: true,
      observeParents: true,
      slidesPerView: 1,
      ...
      navigation: {
        prevEl: '.product-card .arrows-product__arrow_prev',
        nextEl: '.product-card .arrows-product__arrow_next',
      },
    });
  }
  ...
  if (document.querySelector('.slider-product-page')) {
    ...
    //Создаем слайдер
    new Swiper('.slider-product-page__slider', {
      modules: [A11y, Pagination, Thumb],
      observer: true,
      observeParents: true,
      slidesPerView: 1,
      spaceBetween: 20,
...
ymaps_esm
  ...
  .then(maps => {
    const mapContainer = document.querySelector(".contacts-page__map");
    if (mapContainer) {
...
function documentActions(e) {
  ...
  if (targetElement.closest(".catalog__show-filter")) {
    targetElement.closest(".catalog__filters").classList.add("_show-filter");
    document.documentElement.classList.add("lock");
  } else if (targetElement.closest(".top-filter-catalog__close")) {
    targetElement.closest(".catalog__filters").classList.remove("_show-filter");
    document.documentElement.classList.remove("lock");
  }
};
